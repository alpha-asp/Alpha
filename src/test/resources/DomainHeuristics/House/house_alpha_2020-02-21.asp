% in this version, heuristic directives are used in Alpha's new syntax
...
#heuristic cabinetTOthing(C,T) : cabinetDomain(C), not fullCabinet(C), not T assignedThing(T), personTOthing(P,T), not otherPersonTOcabinet(P,C), maxCabinet(MC), W=MC-C+10,     thingLong(T). [W@3]
#heuristic cabinetTOthing(C,T) : cabinetDomain(C), not fullCabinet(C), not T assignedThing(T), personTOthing(P,T), not otherPersonTOcabinet(P,C), maxCabinet(MC), W=MC-C+10, not thingLong(T). [W@2]
% then fill rooms with cabinets:
#heuristic roomTOcabinet(R,C)  : roomDomain(R), not fullRoom(R), cabinet(C), not T assignedCabinet(C), personTOcabinet(P,C), not otherPersonTOroom(P,R), maxRoom(MR), W=MR-R+10. [W@1]
% when all things and cabinets are assigned, close remaining choices:
#heuristic F cabinet(C) : not cabinet(C), cabinetDomain(C). [5]
#heuristic F room(R) : not room(R), roomDomain(R). [5]
#heuristic F cabinetTOthing(C,T) : not cabinetTOthing(C,T), cabinetDomain(C), thing(T). [5]
#heuristic F roomTOcabinet(R,C) : not roomTOcabinet(R,C), roomDomain(R), cabinet(C). [5]