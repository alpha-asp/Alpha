...
public class AggregateRewritingTest {
	...
	private static final String CNT_EQ_CHOICE_ASP = 
			"potential_thing(1..4). "
	...
	@Test
	public void countEqSortingGridWithChoice() {
		Alpha alpha = new Alpha();
		alpha.getConfig().setEvaluateStratifiedPart(false);
		InputProgram input = alpha.readProgramString(CNT_EQ_CHOICE_ASP);
		NormalProgram normalized = alpha.normalizeProgram(input);
		System.out.println(normalized);
		Set<AnswerSet> answerSets = alpha.solve(normalized, (p) -> true).collect(Collectors.toSet());
		TestUtils.assertAnswerSetsEqualWithBase(
	...
	@Test
	public void countEqCountingGridWithChoice() {
		Alpha alpha = new Alpha();
		alpha.getConfig().setEvaluateStratifiedPart(false);
		InputProgram input = alpha.readProgramString(CNT_EQ1_ASP);
		NormalProgram normalized = alpha.normalizeProgram(input);
