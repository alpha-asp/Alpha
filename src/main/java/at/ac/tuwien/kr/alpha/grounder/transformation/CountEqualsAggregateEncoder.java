...
public class CountEqualsAggregateEncoder extends AbstractAggregateEncoder {
	...
	private static final ST CNT_EQ_LITERAL_ENCODING = new ST(
				...
				+ "<leq>(<value_var>) :- <value_leq_cnt_lit>, <cnt_candidate>(<value_var>)."
	...
	protected List<BasicRule> encodeAggregateResult(AggregateLiteral lit, AggregateRewritingContext ctx) {
		String aggregateId = ctx.getAggregateId(lit);
		...
		ctx.registerAggregateLiteral(candidateLeqCount);
		...
		List<BasicRule> candidateLeqEncoding = candidateLeqEncoder.encodeAggregateLiteral(candidateLeqCount, ctx);
		...
		encodingTemplate.add("aggregate_result", ctx.getAggregateOutputAtom(lit).getPredicate().getName());
		encodingTemplate.add("leq", aggregateId + "_leq");
		encodingTemplate.add("cnt_candidate", aggregateId + "_candidate");
		encodingTemplate.add("value_var", valueVar.toString());
		encodingTemplate.add("value_leq_cnt_lit", ctx.getAggregateOutputAtom(candidateLeqCount).toString());
